            ...
            if strong and table.find('a', href=lambda x: x and 'archive.php' in x):
                ...
                if feed_id:
                    current_feed_data = {
                        ...
                        'title': title,
                        'status': status,
                        'archive_url': f"{self.BASE_URL}{archive_href}" if archive_href.startswith('/') else archive_href,
                        'stream_pls_url': f"{self.BASE_URL}{pls_href}" if pls_href.startswith('/') else pls_href,
                        'frequencies': [],
                    }
